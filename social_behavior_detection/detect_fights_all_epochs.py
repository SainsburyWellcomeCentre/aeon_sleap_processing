...
def create_slurm_script(
    ...
    script = f"""#!/bin/bash
#SBATCH --job-name=fight_detect                # job name
...
python -u detect_fights.py --root {root} --start $START --end $END $EPOCH_CMD --parameters '{parameters_json}' --skeleton '{skeleton_json}' --video_config '{video_config_json}' --output_dir {output_dir}
EXIT_STATUS=$?
if [ $EXIT_STATUS -eq 0 ]; then
    ...
    echo -e "$EMAIL_BODY" | mail -s "Fight Detection Job ${{SLURM_JOB_ID}} Failed" $USER_EMAIL
fi
date +"%Y-%m-%d %H:%M:%S"
"""
    return script
...
def main():
    ...
    parser.add_argument(
        "--skeleton", 
        help="A mapping of the required nodes to their corresponding names in your SLEAP project as a JSON string.",
        required=False,
        type=parse_dict,
        default={
            "nose": "nose",
    ...
    script_filename = f"{acquisition_computer}_{exp_name}_fight_detection.sh"
